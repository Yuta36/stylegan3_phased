...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, zoom_data=args.zoom_data)
        if rank == 0:
...
# For phased paper, zoom dataset for validation.
def calc_metrics(network_pkl, metrics, data, mirror, gpus, verbose, zoom_data):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, zoom_data=zoom_data)
